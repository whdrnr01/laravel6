...
@section('content')
    ...
    <div class="mt-4 px-64">
        <form action="/tasks/{{ $task->id }}" method="POST">
            ...
            <input type="text" name="title" id="title" class="border-2 rounded p-2" value="{{ old('title')?old('title'):$task->title }}">

            <label for="body" class="block font-bold">Body</label>
            <textarea name="body" id="body" cols="30" rows="10" class="p-2 border-2 border-gray-400 rounded">{{ old('body')?old('body'):$task->body }}</textarea>
